                        ...
                        {isPrinting ? (
                            ...
                            <ResponsiveContainer width="100%" height="100%" minWidth={0}>
                                <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
                                    ...
                                    <YAxis stroke="#94a3b8" fontSize={11} tickFormatter={(v) => isValuesVisible ? `R$ ${v/1000}k` : '••••'} />
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {monthlyData.map((data, index) => (
                                        <tr key={index} className="border-b border-slate-700 print:border-black">
                                            ...
                                            <td className="px-4 py-2 text-right">{displayValue(data.despesaCom)}</td>
                                            <td className="px-4 py-2 text-right">{displayValue(data.despesaSem)}</td>
                                            <td className="px-4 py-2 text-right font-bold text-red-400 print:text-black">{displayValue(data.totalDespesa)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                                <tfoot className="bg-slate-700/50 print:bg-slate-100 font-bold">
                                    <tr className="border-t-2 border-slate-500 print:border-black">
                                        ...
                                        <td className="px-4 py-2 text-right" colSpan={3}>{displayValue(totalDespesa)}</td>
                                    </tr>
                                    <tr className="text-xs text-slate-400 print:text-black">
                                        <td className="px-4 py-1">MÉDIA MENSAL</td>
                                        <td className="px-4 py-1 text-right" colSpan={3}>{displayValue(mediaDespesaTotal)}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody>
                                    {monthlyData.map((data, index) => (
                                        <tr key={index} className="border-b border-slate-700 print:border-black">
                                            <td className="px-4 py-2 font-semibold">{data.month}</td>
                                            <td className="px-4 py-2 text-right">{displayValue(data.faturamentoCom)}</td>
                                            <td className="px-4 py-2 text-right">{displayValue(data.faturamentoSem)}</td>
                                            <td className="px-4 py-2 text-right font-bold text-green-400 print:text-black">{displayValue(data.totalFaturamento)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                                <tfoot className="bg-slate-700/50 print:bg-slate-100 font-bold">
                                    <tr className="border-t-2 border-slate-500 print:border-black">
                                        <td className="px-4 py-2">TOTAL ACUMULADO</td>
                                        <td className="px-4 py-2 text-right" colSpan={3}>{displayValue(totalFaturamento)}</td>
                                    </tr>
                                    <tr className="text-xs text-slate-400 print:text-black">
                                        <td className="px-4 py-1">MÉDIA MENSAL</td>
                                        <td className="px-4 py-1 text-right" colSpan={3}>{displayValue(mediaFaturamentoTotal)}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                                ...
                                {monthlyData.map((data, index) => {
                                    ...
                                    return (
                                        <tr key={index} className="border-b border-slate-700 print:border-black">
                                            ...
                                            <td className="px-4 py-2 text-right text-red-300 print:text-black">{displayValue(data.totalDespesa)}</td>
                                            <td className="px-4 py-2 text-right text-green-300 print:text-black">{displayValue(data.totalFaturamento)}</td>
                                            <td className={`px-4 py-2 text-right font-black ${totalMes < 0 ? 'text-red-500' : 'text-green-500'} print:text-black`}>
                                                {displayValue(totalMes)}
                                            </td>
                                        </tr>
                                    );
                                })}
